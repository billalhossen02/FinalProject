...
def payment(request):
    ...
    store_id = "abc60fcefe3d5cfd"
    API_key = "abc60fcefe3d5cfd@ssl"
...
def complete(request):
    if request.method == 'POST' or request.method == "post":
        ...
        if status == 'VALID':
            ...
            messages.success(request, f"Your Payment Completed Succefully")
            return HttpResponseRedirect(reverse("App_Payment:purchase", kwargs={'val_id':val_id, 'tran_id':tran_id},))
        elif status =="FAILED":
            messages.warning(request, f"Your Payment Failed! Try Again")
